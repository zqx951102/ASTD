...
class MixAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, dwconv_kernel_size, num_heads, qkv_bias=True, qk_scale=None,
        ...
        self.channel_interaction = nn.Sequential(     #通道交互的 代码
        ...
        self.qkv = nn.Linear(dim // 2, dim // 2 * 3, bias=qkv_bias)  # 在Attention分支通道数减半   生成QKV
        self.attn_drop = nn.Dropout(attn_drop)
        self.spatial_interaction = nn.Sequential(  #空间交互
    ...
    def _get_rel_pos(self):
        """
            ...
            Args:
                window_size (tuple[int]): window size
        """
        coords_h = torch.arange(self.window_size[0])  #分别创建了一个包含0到窗口高度（self.window_size[0]）和0到窗口宽度（self.window_size[1]）范围内整数的张量。这表示窗口内可能的行和列索引。
        coords_w = torch.arange(self.window_size[1])
        coords = torch.stack(torch.meshgrid([coords_h, coords_w]))  # [2, Mh, Mw]  #使用torch.meshgrid函数将coords_h和coords_w的笛卡尔积堆叠起来，生成了一个形状为 [2, Mh, Mw] 的张量，其中 Mh 表示窗口高度，Mw 表示窗口宽度。这个张量用来表示窗口内每个像素点的绝对位置坐标。
        coords_flatten = torch.flatten(coords, 1)  # [2, Mh*Mw] 建立了一个绝对位置矩阵  将 coords 张量展平为一个形状为 [2, Mh*Mw] 的张量，将每个像素点的绝对位置坐标表示成一个2维坐标。
        # [2, Mh*Mw, 1] - [2, 1, Mh*Mw] 广播机制---前者将第3个维度复制Mh*Mw次，后者将第2个维度复制Mh*Mw次
        # 以当前像素点的绝对位置索引减去其它像素点的绝对位置索引，就能得到这个像素点的相对位置索引   通过广播机制，将 coords_flatten 张量与其自身的每个列向量相减，从而计算出每个像素点与其他像素点之间的相对位置索引。结果是一个形状为 [2, Mh*Mw, Mh*Mw] 的张量，其中第一个维度表示x和y方向的相对坐标。
        relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]  # [2, Mh*Mw, Mh*Mw]
        relative_coords = relative_coords.permute(1, 2, 0).contiguous()  # [Mh*Mw, Mh*Mw, 2] 得到最终的相对位置索引 对 relative_coords 进行维度重排，将其变为形状 [Mh*Mw, Mh*Mw, 2]，这个张量包含了最终的相对位置索引信息。
        relative_coords[:, :, 0] += self.window_size[0] - 1  # shift to start from 0 这两行代码将相对位置索引中的x和y坐标偏移，以便从0开始。这是因为之前的计算是以窗口左上角的像素点为基准的。
        relative_coords[:, :, 1] += self.window_size[1] - 1
        relative_coords[:, :, 0] *= 2 * self.window_size[1] - 1  #将x坐标乘以 2 * self.window_size[1] - 1，这是为了在相对位置索引中引入更大的差异，以便更好地区分不同像素点之间的相对位置。
        return relative_coords
#最终，函数返回一个形状为 [Mh*Mw, Mh*Mw, 2] 的张量 relative_coords，其中包含了窗口内每个像素点之间的相对位置索引信息。这对于处理像素级别的图像数据或者词级别的文本数据非常有用，因为它可以用于建立像素或单词之间的关联关系，例如在注意力机制中使用。
    def forward(self, x, H, W, mask=None):
        ...
        x_atten = self.proj_attn_norm(self.proj_attn(x))   #attention 中通道数减半
        # proj_cnn(): -> [B*num_windows, N, C]
        x_cnn = self.proj_cnn_norm(self.proj_cnn(x))  #cnn不减半
        # window_reverse2(): -> [B, C, H, W]
        x_cnn = window_reverse2(x_cnn, self.window_size, H, W)  #Windows reverse to feature map.
        ...
        x_cnn = self.projection(x_cnn)   #self.projection = nn.Conv2d(dim, dim // 2, kernel_size=1)  用1*1卷积进行投影
        ...
        # channel interaction
        # reshape -> [B, 1, num_heads, 1, C/num_heads] --- C: C/2  对输入张量应用 sigmoid 激活函数，将其值限制在 (0, 1) 范围内。
        ...
        v = v.reshape(x_cnn2v.shape[0], -1, self.num_heads, N, C // self.num_heads)  #x_cnn2v.shape[0] 他的大小
        # *: -> [B, num_heads, num_heads, N, C/num_heads] --- C: C/2
        v = v * x_cnn2v  #呈上一个动态的权重 其维度与 v 相同，实际上是将 v 中的每个通道与 x_cnn2v 中的相应通道相乘。
        ...
        q = q * self.scale  # Q/sqrt(dk)    #注意力的公式
        attn = (q @ k.transpose(-2, -1))  # Q*K^{T} / sqrt(dk)
#综合起来，这段代码的目的是通过引入相对位置偏置来增强自注意力机制，以更好地捕捉输入序列或空间数据中不同位置之间的关联性。这对于许多任务，如
# 自然语言处理和计算机视觉中的序列和图像处理任务，都是很有用的。相对位置偏置可以帮助模型更好地理解不同位置之间的重要性和关系。
        ...
        if mask is not None:
            ...
            attn = self.softmax(attn)  #如果没有mask操作那就进行 softmax操作
#总的来说，这段代码的目的是根据输入的注意力分数（attn）计算注意力权重，用于在自注意力机制中对输入数据进行加权聚合。如果存在 mask，则
# 还会考虑掩盖操作以限制注意力的范围。最终，attn 包含了 softmax 后的注意力权重张量。
        # [B*num_windows, num_heads, N, N]
        attn = self.attn_drop(attn)   #加入一个Drop操作
        ...
        x_spatial = window_reverse2(x_atten, self.window_size, H, W)#将window还原成 特征图的形式
        # conv: -> [B, C/8, H, W] --- C: C/2
        # conv: -> [B, 1, H, W]    conv1*1 bn gelu conv1*1 在去sigmoid
        spatial_interaction = self.spatial_interaction(x_spatial)  #空间交互
        # sigmoid: -> [B, 1, H, W]
        # * -> [B, C, H, W] --- C: C/2
        x_cnn = torch.sigmoid(spatial_interaction) * x_cnn  #这个x_cnn 是投影后的值  和 空间交互后的值进行  点乘
        x_cnn = self.conv_norm(x_cnn) #然后在跟一个Norm操作
        ...
        x = torch.cat([x_cnn, x_atten], dim=2)  #连接两个特征
        # proj: -> [num_windows*B, N, C]
        x = self.proj(x)   # # final projectio  self.proj = nn.Linear(dim, dim)  self.proj_drop = nn.Dropout(proj_drop)
        x = self.proj_drop(x)
        return x
